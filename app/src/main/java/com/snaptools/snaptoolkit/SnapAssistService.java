...
public class SnapAssistService extends AccessibilityService implements android.hardware.SensorEventListener {
    ...
    private long lastNearAt;
    private int waveCount;
    private long firstWaveAt;
    private long lastVolumeUpAt;
    ...
    protected void onServiceConnected() {
        ...
        if (proximitySensor != null) {
            toast(Strings.get("assist_hint_wave"));
        } else {
            toast(Strings.get("assist_no_proximity"));
            toast(Strings.get("assist_hint_volume"));
        }
    }
    ...
    protected boolean onKeyEvent(android.view.KeyEvent event) {
        ...
        if (now - lastVolumeUpAt <= 700) {
            lastVolumeUpAt = 0;
            toast(Strings.get("assist_trigger_volume"));
            activateAssist();
            return true;
        }
        lastVolumeUpAt = now;
    ...
    public void onSensorChanged(android.hardware.SensorEvent event) {
        if (event == null || event.sensor == null || event.sensor.getType() != android.hardware.Sensor.TYPE_PROXIMITY) {
            return;
        }
        float value = event.values[0];
        boolean isNear = value < proximitySensor.getMaximumRange();
        long now = android.os.SystemClock.uptimeMillis();

        if (isNear && !lastNear) {
            lastNearAt = now;
        } else if (!isNear && lastNear) {
            long waveDuration = now - lastNearAt;
            if (waveDuration <= 900) {
                if (waveCount == 0 || now - firstWaveAt > 2500) {
                    firstWaveAt = now;
                    waveCount = 1;
                } else {
                    waveCount++;
